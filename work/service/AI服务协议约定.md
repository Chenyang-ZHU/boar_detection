...
|------|------|
| 格式 | JPEG、PNG、BMP |
| 尺寸 | **必须为 640×640 像素**（宽×高），其他尺寸一律拒绝 |
| 颜色空间 | RGB 或 BGR 均可 |
| 文件大小 | 建议控制在 500KB 以内（640×640 的 JPEG 通常在 100~300KB） |
...
|------|------|------------|---------------|
| 0 | 成功 | 200 | — |
| 40001 | 不支持的图片格式 | 200 | 检查图片扩展名和编码格式，确保为 JPEG/PNG/BMP |
| 40004 | 图片尺寸不是 640×640 | 200 | 将图片缩放到 **640×640** 再重试 |
...
- 传入图片**必须为 640×640 像素**，服务端不做自动缩放
- 调用方需要在发送前自行将图片缩放到 640×640
- 建议缩放时保持宽高比，不足部分用黑色填充（letterbox 方式），以保证目标不变形
- 如果传入非 640×640 的图片，服务端将返回 `code: 40004`
